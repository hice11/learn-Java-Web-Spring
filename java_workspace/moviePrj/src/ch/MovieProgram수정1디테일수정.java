...
public class MovieProgram수정1디테일수정 {
	...
	private int[] selectInput() {
		...
		do {
			...
			System.out.print("\n영화를 선택하세요: ");
			b = sc.nextInt(); 
		}while( b < 1 || 3 < ScreenData.movies.length );
		//날짜 선택
		do {
			System.out.println();
			...
			System.out.print("\n날짜를 선택하세요: ");
			c = sc.nextInt(); 
		}while( c < 1 || 3 < ScreenData.dates.length );
		//시간 선택 + 상영관 정보
		do {
			System.out.println();
			...
			d = sc.nextInt(); 
			System.out.println();
		}while( d < 1 || 3 < ScreenData.times.length );
	...
	public static void main(String[] args) {
		MovieProgram수정1디테일수정 mProgram = new MovieProgram수정1디테일수정();
