...
public class MovieProgram예외처리 {
	...
	private int[] selectInput() {
		...
		while (true) {
			...
			if( 0 < b || b <= ScreenData.movies.length )
				break;
			else {
				System.out.println("\n잘못 입력하셨습니다.\n");
			}
		...
		while (true) {
			...
			if( 0 < c || c <= ScreenData.dates.length )
				break;
			else {
				System.out.println("\n잘못 입력하셨습니다.\n");
			}
		...
		while (true) {
			...
			if( 0 < d || d <= ScreenData.dates.length )
				break;
			else {
				System.out.println("\n잘못 입력하셨습니다.\n");
			}
	...
	// 시간 선택 + 상영관 정보
	private int selectTime(int a, int b, int c) {
		System.out.println();
		for( int  i =0 ;  i< ScreenData.times.length ; i++){
			System.out.println( (i+1)+ "."+ScreenData.times[i]+ScreenData.theaters[a-1][b-1][c-1][i]);}
		System.out.print("\n시간을 선택하세요: ");
	...
	// 선택 오류 메시지
	private void alert(Object obj, int num, String[] arr) {
		while (true) {
	...
	*/
	...
	public static void main(String[] args) {
		MovieProgram예외처리 mProgram = new MovieProgram예외처리();
